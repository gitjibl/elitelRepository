define([
    ...
    return declare("el.Layer.ClusterLayer", [GraphicsLayer], {
        constructor: function (options) {
            ...
            //this._singleTemplate = options.singleTemplate || new PopupTemplate({ "title": "", "description": "{*}" }); //esri/InfoTemplate
        ...
        _setMap: function (map, surface) {
            ...
            var _getClusterResolution = function () {
                var extentWidth = map.spatialReference.wkid != "4326" ?
                    webMercatorUtils.webMercatorToGeographic(map.extent).getWidth() : map.extent.getWidth();
                return extentWidth / map.width;
            };
            //this._clusterResolution = map.extent.getWidth() / map.width; // probably a bad default...
            this._clusterResolution = _getClusterResolution();
        ...
        onClick: function (e) {
            ...
            if (singles.length > this._maxSingles) {
                ...
                //20151206 add by zhangtao
                var infoTemplate;
                if (e.graphic.attributes.clusterType == "cluster") {
                    infoTemplate = this._clusterTemplate(e, e.graphic.attributes, this._singles);
                } else if (e.graphic.attributes.clusterType == "single") {
                    infoTemplate = this._singleTemplate(e, e.graphic.attributes);
                } else if (e.graphic.attributes.clusterType == "point") {
                    infoTemplate = this._pointTemplate(e, e.graphic.attributes);
                }

                if (infoTemplate) {
                    //this.infoTemplate = infoTemplate;
                    this._map.infoWindow.setTitle(infoTemplate.title);
                    this._map.infoWindow.setContent(infoTemplate.content);
                }
            }
        },
        _clusterTemplate: function (e, attributes, singles) {
            return new InfoTemplate("clusterTemplate", "<h1>clusterTemplate[" + singles.length + "]</h1>");
        },
        _singleTemplate: function (e, attributes) {
            return new InfoTemplate("singleTemplate", "<h1>singleTemplate</h1>");
        },
        _pointTemplate: function (e, attributes) {
            return new InfoTemplate("pointTemplate", "<h1>pointTemplate</h1>");
        },
        // internal methods 
        ...
        _showCluster: function (c) {
            if (c.attributes.clusterCount == 1) {
                if (c.symbols) {
                    ...
                    for (var i = 0; i < c.symbols.length; i++) {
                        var attributes = lang.clone(c.attributes);
                        attributes.clusterType = "point";
                        this.add(new Graphic(point, c.symbols[i], attributes));
                    }
                } else {
                    console.log("didn't find attribute symbols: ", c);
                }
                return;
            }
            ...
            var point = new Point(c.x, c.y, this._sr);
            var attributes = lang.clone(c.attributes);
            attributes.clusterType = "cluster";
            this.add(new Graphic(point, null, attributes));
            ...
            this.add(new Graphic(point, label, attributes));
        },
        _addSingles: function (singles) {
            // add single graphics to the map
            arrayUtils.forEach(singles, function (p) {
                var attributes = lang.clone(p.attributes);
                attributes.clusterType = "single";
                var g = new Graphic(new Point(p.x, p.y, this._sr), this._singleSym, attributes);
                this._singles.push(g);
                if (this._showSingles && p.attributes.clusterCount != 1) {
                    this.add(g);
                }
            }, this);
            //20151204 add by zhangtao esri/dijit/Popup => esri/InfoWindowBase
            //this._map.infoWindow.setFeatures(this._singles);
        },
        _updateClusterGeometry: function (c) {
            // find the cluster graphic
            var points = arrayUtils.filter(this.graphics, function (g) {
            ...
            for (var i = 0; i < points.length; i++) {
                //delete all graphics include label
        ...
        _updateLabel: function (c) {
            ...
            var labels = arrayUtils.filter(this.graphics, function (g) {
                return g.symbol &&
            ...
            var attributes = lang.clone(c.attributes);
            attributes.clusterType = "cluster";
            this.add(new Graphic(new Point(c.x, c.y, this._sr), label, attributes));
